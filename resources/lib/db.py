...
class cDb(object):
    ...
    def _create_tables(self, dropTable=''):
        ...
        sql_create = "CREATE TABLE IF NOT EXISTS watchedviews ("\
                     "addon_id integer PRIMARY KEY AUTOINCREMENT, "\
                     "tmdb_id TEXT, "\
                     "title_id TEXT, "\
                     "title TEXT, "\
    ...
    def insert_watched(self, meta):
        ...
        #titleWatched = xbmc.getInfoLabel('ListItem.Property(sCleanTitle)')
        titleWatched = self.str_conv(titledecode)
        titleWatched = re.sub(' ', '', titleWatched) 
        cat = meta['cat'] if 'cat' in meta else '1'
        saison = meta['season'] if 'season' in meta else ''
        sTmdbId = oInputParameterHandler.getValue('sTmdbId') #meta['sTmdbId'] if 'sTmdbId' in meta else ''
        ...
        try:
            ...
            pass
        try:
            ex = 'INSERT INTO Watched (tmdb_id, title_id, title, siteurl, site, fav, cat, season) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    ...
    def get_watched(self):
        sql_select = "SELECT * FROM watched group by title order by addon_id DESC"
    ...
    def del_watched(self, meta):
        ...
        if not sTitleWatched:       # delete a category or all
            sql_delete = "DELETE FROM watched"
            if 'cat' in meta:
                sql_delete += " where cat = '%s'" % meta['cat']
        else:
            sql_delete= "DELETE FROM watched WHERE title_id = '%s'" % sTitleWatched
            if 'cat' in meta:
    ...
    def insert_watchedviews(self, meta):
        ...
        try:
            ex = 'INSERT INTO watchedviews (title, siteurl, site, fav, cat, icon, fanart) VALUES (?, ?, ?, ?, ?, ?, ?)'
            self.dbcur.execute(ex, (title, siteurl, meta['site'], meta['fav'], meta['cat'], sIcon, meta['fanart']))

            self.db.commit()

            dialog().VSinfo(addon().VSlang(30042), meta['title'], 4)
            VSlog('SQL INSERT watchedviews Successfully - ' + meta['title'])
        except Exception as e:
            if 'UNIQUE constraint failed' in str(e):
    ...
    def get_watchedviews(self):

        sql_select = 'SELECT * FROM watchedviews order by addon_id desc'
    ...
    def del_watchedviews(self, sSiteUrl='', sMovieTitle='', sCat='', sAll=False):

        sql_delete = None

        # Tous supprimer
        if sAll:
            sql_delete = 'DELETE FROM watchedviews;'
        ...
        # Supprimer un bookmark selon son titre
            ...
            sql_delete = "DELETE FROM watchedviews WHERE siteurl = '%s' AND title = '%s'" % (siteUrl, title)

        # Supprimer un bookmark selon son url
        elif sSiteUrl:
            siteUrl = QuotePlus(sSiteUrl)
            sql_delete = "DELETE FROM watchedviews WHERE siteurl = '%s'" % siteUrl
        ...
        # Supprimer toute une catégorie
            ...
            if sCat not in catList:
                ...
                if sCat not in catList:
                    catList = ('0', sCat)
            sql_delete = "DELETE FROM watchedviews WHERE cat in %s" % str(catList)
        ...
        if sql_delete:
            ...
            try:
                ...
                if not update and sSiteUrl and sMovieTitle:
                    ...
                    return self.del_watchedviews(sSiteUrl)
